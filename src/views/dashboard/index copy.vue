      ...
      <el-collapse-item name="1">
        ...
        <el-tabs v-model="activeName" @tab-click="handleSwitchTabs">
            ...
            <el-card class="box-card" style="margin-top:30px">
              ...
              <el-table :data="NewPriitems" element-loading-text="Loading" border stripe fit highlight-current-row>
                ...
                <el-table-column class-name="status-col" label="操作3" width="110" align="center">
                  ...
                  <template slot-scope="scope">
                    <el-button @click="jumpForm(scope.row.gtalename)">上传</el-button>
                  </template>
          ...
          <el-tab-pane name="second">
            ...
            <el-table row-key="id" :expand-row-keys="privateExpands" :data="MapPriitems" style="width: 100%"
              @expand-change="expandPrivateChange">
              ...
              <el-table-column label="原文件" prop="sourceFile" align="center">
              </el-table-column>
              ...
              <el-table-column align="right">
                ...
                <template slot-scope="scope">
                  <el-button disabled size="medium" @click="handleEditMapping(scope.$index, scope.row)">编辑</el-button>
                  <el-button disabled size="medium" @click="handleExportMapping(scope.$index, scope.row)">导出</el-button>
      ...
      <el-collapse-item name="2">
        ...
        <el-tab-pane name="first">
          ...
          <el-card class="box-card">
            ...
            <el-pagination @current-change="handlePubilcCurrentChange" :current-page.sync="publicPage" :page-size="p2Size"
         ...
         <el-tab-pane name="second">
            <span slot="label" class="tableTitleClass">映射</span>
            <el-button disabled style="margin-top: 10px" @click="upLoadMapping">导入映射文件</el-button>
            <el-table :data="MapPubitems" row-key="id" :expand-row-keys="publicExpands"  style="width: 100%"
              @expand-change="expandPublicChange">
              <el-table-column type="expand">
                <template slot-scope="props">
                  <el-table :data="props.row.info" style="width: 100%" row-key="sourceField" highlight-current-row
                    :row-style="tableRowClassName" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
            ...
            <el-pagination @current-change="handleMappingPubilcCurrentChange" :current-page.sync="MappingpublicPage" :page-size="p2Size"
              layout="total, prev, pager, next, jumper" :total="MappingpublicTotal">
            </el-pagination>
          </el-tab-pane>
...
import {getPublicMappings} from '@/api/mapping'
...
export default {
  ...
  methods: {
    ...
    async getPublicHistoricalMapping() {
      this.historicalPublicMappings = [];
      var mapping_response = await getPublicMappings()
      // var mapping_response = eval("(" + mapping_response_str + ")")
      var mapping_data = mapping_response.data
      console.log(mapping_data)
      var temp_historicalPublicMappings = [];
      var his_id = 1;
      for (var index in mapping_data) {
        var item_mappings = {}
        var item = mapping_data[index]
        item_mappings['mappingName'] = '映射文件-' + item.Mapping_tableName
        item_mappings['sourceFile'] = item.Mapping_tableName
        item_mappings['targetFile'] = item.Mapping_tableName
        item_mappings['timestamp'] = this.publicFiles[his_id].gTableTime;
        item_mappings['id'] = his_id;
        his_id ++;
        var relations = JSON.parse(item.Mapping_relation)
        var info = []
        var i = 1;
        for (var relation in relations){
          var item_relation = {}
          item_relation['id'] = i;
          item_relation['sourceField'] = relation
          item_relation['targetField'] = relations[relation]
          info.push(item_relation)
          i ++ ;
        }
        var replacement = JSON.parse(item.Mapping_hash)
        if (replacement != {}){
          for(var replace_key in replacement){
            for(var j = 0; j < info.length; j ++){
              if(replace_key == info[j].targetField){
                var childrens = []
                for (var content_key in replacement[replace_key][0]){
                  var children = {}
                  children['isChildren'] = "true"
                  children['sourceField'] = replacement[replace_key][0][content_key]
                  children['targetField'] = content_key
                  childrens.push(children)
                }
                info[j]['children'] = childrens
              }
            }
          }
        }
        item_mappings['info'] = info
        temp_historicalPublicMappings.push(item_mappings)
      }
      this.historicalPublicMappings = temp_historicalPublicMappings
      this.MappingpublicTotal=this.historicalPublicMappings.length
    ...
    // 获取个人历史映射，并将数据进行转换
    getPrivateHistoricalMapping() {
      var temp_historicalPrivateMapping = []
      for(var k = 20; k < 35; k ++){
        temp_historicalPrivateMapping.push(this.historicalPublicMappings[k])
      }
      this.historicalPrivateMapping = temp_historicalPrivateMapping
      ...
      console.log(this.historicalPrivateMapping)
    },
    // 关闭后清空数据
    closeCheckOrExport() {
      this.gridData = []
    ...
    // 导出映射文件
    handleExportMapping(index, row) {

    },
    // 编辑映射文件
    handleEditMapping(index, row) {

    },
    // 删除映射文件
    handleDeletePublicMapping(index, row) {
      var r = confirm("是否删除");
      if (r == true) {
          this.MapPubitems.splice(index, 1)
      }
    },
    handleDeletePrivateMapping(index, row) {
      var r = confirm("是否删除");
      if (r == true) {
          this.MapPriitems.splice(index, 1)
      }
    },
    // 更换个人文件的标签页
    ...
    async fetchData(id) {
      ...
      this.publicFiles = (await getList()).data.sort(function (o1, o2) { return o1.time > o2.time ? 1 : -1 })
    ...
    handleMappingPrivateCurrentChange(val) {
      ...
      console.log(this.historicalPrivateMapping)
      console.log(this.historicalPrivateMapping.slice((this.MappingprivatePage - 1) * this.p2Size, this.MappingprivatePage * this.p2Size,))
      this.historicalPrivateMappingTemp=this.historicalPrivateMapping.slice((this.MappingprivatePage - 1) * this.p2Size, this.MappingprivatePage * this.p2Size,)
